        ...
        <AnimatePresence mode="wait">
          ...
          {view === 'select' && (
            ...
            >
              ...
              {/* Progression globale */}
              <div className={cn('grid grid-cols-2 gap-10 max-w-3xl w-full mx-auto', !currentStory && 'opacity-40 pointer-events-none')}>
                {creationTypes.map((type) => (
                  <Highlightable key={type.id} id={type.id === 'image' ? 'studio-type-image' : 'studio-type-video'} subtle>
                    <motion.button
                      onClick={() => handleSelectType(type.id)}
                      className="relative glass rounded-3xl p-10 text-center hover:border-aurora-500/30 transition-all group overflow-hidden w-full"
                      initial={{ opacity: 0, y: 20 }}
                      animate={{ opacity: 1, y: 0 }}
