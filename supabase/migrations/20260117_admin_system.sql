-- ============================================
-- SYSTÈME D'ADMINISTRATION MULTI-FAMILLES
-- ============================================

-- Table des familles (chaque client = une famille)
CREATE TABLE IF NOT EXISTS families (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  name TEXT NOT NULL,
  code TEXT UNIQUE NOT NULL, -- Code unique pour identifier la famille (ex: DUPONT-2026)
  owner_email TEXT NOT NULL, -- Email du parent principal
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
  is_active BOOLEAN DEFAULT true
);

-- Table de configuration par famille (clés API, préférences)
...
-- Table super_admins (toi et tes collaborateurs)
...
CREATE POLICY "super_admins_families_all" ON families
...
CREATE POLICY "super_admins_config_all" ON family_config
  FOR ALL TO authenticated
  USING (EXISTS (SELECT 1 FROM super_admins WHERE user_id = auth.uid()));

DROP POLICY IF EXISTS "super_admins_members_all" ON family_members;
CREATE POLICY "super_admins_members_all" ON family_members
  FOR ALL TO authenticated
  USING (EXISTS (SELECT 1 FROM super_admins WHERE user_id = auth.uid()));

-- Parents peuvent voir leur propre famille
DROP POLICY IF EXISTS "parents_view_own_family" ON families;
CREATE POLICY "parents_view_own_family" ON families
...
-- Parents peuvent voir la config de leur famille (SANS les clés sensibles via view)
DROP POLICY IF EXISTS "parents_view_own_config" ON family_config;
CREATE POLICY "parents_view_own_config" ON family_config
  FOR SELECT TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM family_members 
      WHERE family_members.family_id = family_config.family_id 
      AND family_members.user_id = auth.uid()
      AND family_members.role = 'parent'
    )
  );

-- Parents peuvent gérer les membres de leur famille
DROP POLICY IF EXISTS "parents_manage_own_members" ON family_members;
CREATE POLICY "parents_manage_own_members" ON family_members
  FOR ALL TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM family_members AS parent_check
      WHERE parent_check.family_id = family_members.family_id 
      AND parent_check.user_id = auth.uid()
      AND parent_check.role = 'parent'
    )
  );

-- Membres peuvent voir les autres membres de leur famille
DROP POLICY IF EXISTS "members_view_own_family_members" ON family_members;
CREATE POLICY "members_view_own_family_members" ON family_members
  FOR SELECT TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM family_members AS self
      WHERE self.family_id = family_members.family_id 
      AND self.user_id = auth.uid()
    )
  );

-- Super admins table - only super admins can see
...
-- Trigger pour updated_at sur families
...
-- Trigger pour updated_at sur family_config
...
-- Trigger pour updated_at sur family_members
...
-- Fonction pour générer un code famille unique
...
DECLARE
  ...
  counter INT := 0;
BEGIN
  -- Nettoyer le nom et créer le code de base
...
-- Fonction pour obtenir la config d'une famille pour un utilisateur
